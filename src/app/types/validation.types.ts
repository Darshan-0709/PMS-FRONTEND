...
export type ValidationMessages = {
  [key: string]:
    | string
    | ((control: AbstractControl, label?: string) => string);
};
...
export const defaultValidationMessages: ValidationMessages = {
  ...
  maxlength: (c, label = 'This field') =>
    `${label} must be at most ${
      c.getError('maxlength')?.requiredLength
    } characters.`,
  email: () => 'Please enter a valid Email.',
  server: (c) => c.getError('server'),
  min: (c, label = 'This field') =>
    `${label} must be at least ${c.getError('min')?.min}.`,
  max: (c, label = 'This field') =>
    `${label} must be at most ${c.getError('max')?.max}.`,
