      ...
      <div class="flex flex-1 items-center space-x-2">
        ...
        <div class="relative flex-grow max-w-md">
          ...
          <input
            ...
            placeholder="Search..."
            [attr.aria-label]="'Search in ' + (activeColumn()?.header || 'all searchable columns')"
          />
        ...
        <div class="flex items-center">
          ...
          />
          <label for="case-sensitive" class="ml-2 text-sm text-gray-600"> Case sensitive </label>
        </div>
      </div>
    </div>
  `,
...
export class TableSearchComponent<T> {
  @Input() set searchConfig(config: SearchConfig<T> | undefined) {
    if (config) {
      ...
      this.selectedColumnField = config.fields?.length === 1 ? config.fields[0] : '';
  ...
  activeColumn = computed(() => {
    ...
    return this.searchableColumns().find(col => col.field === this.selectedColumnField) || null;
  });
