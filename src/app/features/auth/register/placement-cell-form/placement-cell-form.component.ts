...
export class PlacementCellFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const placementCellForm = new FormGroup({
      ...
      degreeNames: new FormControl<string[]>([], Validators.required),
    ...
    placementCellForm.valueChanges.subscribe((val) => {
      if (
        val.branchName !== undefined &&
        val.degreeNames !== undefined &&
        val.domains !== undefined &&
        val.placementCellEmail !== undefined &&
        val.placementCellName !== undefined &&
        val.website !== undefined
      ) {
        this.registerService.setPlacementCellProfile(
          val as PlacementCellProfileData
        );
      }
    });
    if (savedData) {
  ...
  onRemoveDegree(degreeId: string) {
    ...
    const updatedDegrees = currentDegrees.filter(
      (degree: string) => degree !== degreeId
    );
